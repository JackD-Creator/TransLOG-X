...
async function loadData() {
  ...
  const [{ data: arPaid }, { data: poSent }, { data: scfDisb }] = await Promise.all([
    supabase.from('ar_accounts')
      .select('invoice_amount,paid_amount,interest_amount')
    ...
    supabase.from('ar_accounts')
      .select('disbursed_amount,invoice_amount')
      .eq('ksm_tenant_id', tenantId.value)
      .not('disbursement_date', 'is', null)
      .gte('disbursement_date', dateFrom),
  ])

  const paidTotal    = (arPaid ?? []).reduce((s, a) => s + Number(a.paid_amount ?? a.invoice_amount ?? 0), 0)
  ...
  const scfDisbTotal = (scfDisb ?? []).reduce((s, a) => s + Number(a.disbursed_amount ?? 0), 0)

  operatingIn.value = [
    { label: 'Penerimaan dari RS (pelunasan AR)', amount: paidTotal },
    { label: 'Service fee & jasa logistik managed (est.)', amount: paidTotal * 0.02 },
  ]
  operatingOut.value = [
    { label: 'Pembayaran ke Distributor (PO selesai)', amount: totalPO * 0.92 },
    { label: 'Biaya operasional & SDM (est. 4%)', amount: totalPO * 0.04 },
  ]
  financingIn.value = [
    { label: 'Pencairan fasilitas SCF dari Bank', amount: scfDisbTotal },
  ]
  financingOut.value = [
    { label: 'Pelunasan pokok SCF ke Bank', amount: paidTotal * 0.90, sub: 'Pokok' },
  ...
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-[#999] mt-0.5">Arus kas operasional dan pembiayaan SCF KSM</p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-3 gap-4">
      ...
      <div :class="['rounded-xl border p-4 text-center', netCash >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200']">
        ...
        <p class="text-xl font-bold" :class="netCash >= 0 ? 'text-emerald-700' : 'text-red-600'">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="bg-[#f5f5f5] rounded-xl border border-[#e5e5e5] overflow-hidden">
        ...
        <div class="p-5 text-xs space-y-0">
          <p class="text-[10px] font-bold text-emerald-600 uppercase tracking-wide mb-2">Kas Masuk</p>
          <div v-for="i in operatingIn" :key="i.label" class="flex justify-between py-2 border-b border-[#ececec]">
            <span class="text-[#555]">{{ i.label }}</span>
            <span class="text-emerald-700 font-semibold">+{{ fmtRp(i.amount) }}</span>
          </div>
          <div class="flex justify-between py-2 font-bold border-b-2 border-[#e0e0e0] mb-3">
            <span class="text-[#333]">Subtotal Masuk</span>
            <span class="text-emerald-700">+{{ fmtRp(totOpIn) }}</span>
          </div>

          <p class="text-[10px] font-bold text-red-500 uppercase tracking-wide mb-2">Kas Keluar</p>
      ...
      <div class="space-y-4">
        <div class="bg-[#f5f5f5] rounded-xl border border-[#e5e5e5] overflow-hidden">
          ...
          <div class="p-5 text-xs space-y-0">
            <p class="text-[10px] font-bold text-blue-500 uppercase tracking-wide mb-2">Kas Masuk</p>
            <div v-for="i in financingIn" :key="i.label" class="flex justify-between py-2 border-b border-[#ececec]">
              <span class="text-[#555]">{{ i.label }}</span>
              <span class="text-blue-700 font-semibold">+{{ fmtRp(i.amount) }}</span>
            </div>
            <div class="flex justify-between py-2 font-bold border-b-2 border-[#e0e0e0] mb-3">
              <span class="text-[#333]">Subtotal Masuk</span>
              <span class="text-blue-700">+{{ fmtRp(totFinIn) }}</span>
            </div>

            <p class="text-[10px] font-bold text-red-500 uppercase tracking-wide mb-2">Kas Keluar</p>
            <div v-for="i in financingOut" :key="i.label" class="flex justify-between py-2 border-b border-[#ececec]">
        ...
        <div :class="['rounded-xl border p-5 flex items-center justify-between',
          ...
          <div>
            <p class="text-xs font-bold text-[#1a1a1a]">Kenaikan (Penurunan) Bersih Kas</p>
            <p class="text-[11px] mt-0.5" :class="netCash >= 0 ? 'text-emerald-700' : 'text-red-600'">
              {{ netCash >= 0
                ? 'Cashflow positif — KSM menerima lebih dari yang dikeluarkan'
                : 'Cashflow negatif — percepat collection AR atau optimasi PO' }}
            </p>
          </div>
          <p class="text-2xl font-bold ml-4 flex-shrink-0" :class="netCash >= 0 ? 'text-emerald-700' : 'text-red-600'">
            {{ netCash >= 0 ? '+' : '' }}{{ fmtRp(netCash) }}
          </p>
        </div>
      </div>
